...
uniform sampler2D sGVPosition;
uniform sampler2D sGVNormal;
...
void main()
{
	...
	vec3 vPosition = texture( sGVPosition, oTexCoords ).xyz;										// Position in camera space.
	vec3 vNormal = texture( sGVNormal, oTexCoords ).xyz;											// Normal in camera space.
	...
	for( int i = 0; i < KERNEL_SIZE; ++i )
	{
		...
		// Get viewing depth stored at the G-buffer gVPosition texture at the location given by current sample.
		float vDepth = texture( sGVPosition, offset.xy ).z;
